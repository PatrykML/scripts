...
sudo docker-compose exec mongo mongosh --tls --tlsAllowInvalidCertificates admin --eval 'db.createUser({user: "pcappadmin", pwd: passwordPrompt(), roles: [{ role: "readWriteAnyDatabase", db: "admin" },{ role: 'dbAdminAnyDatabase', db: 'admin' }]})'
