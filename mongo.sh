...
sudo docker-compose exec mongo mongosh --tls --tlsAllowInvalidCertificates admin --eval 'db.createUser({user: "pcappadmin", pwd: "'"$PASSWORD_ROOT"'", roles: [{ role: "readWriteAnyDatabase", db: "admin" },{ role: 'dbAdminAnyDatabase', db: 'admin' }]})'
